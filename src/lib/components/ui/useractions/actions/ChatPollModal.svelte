...
<Modal showModal={true} {onClose}>
  ...
  {#snippet body()}
    ...
    <div class="flex flex-col space-y-2">
      ...
      <Input type="text" id="title" placeholder="Enter poll title" bind:value={title} />
      {#if titleErrorMessage}
        <p class="text-xs text-red-500">{titleErrorMessage}</p>
      {/if}
      ...
      <div class="flex items-center justify-between">
        ...
        <Button variant="default" size="sm" on:click={() => (options = [...options, ''])}>
          <Plus size="16" />
        </Button>
      </div>
      ...
      <div class="flex flex-col space-y-2">
        {#each options as _, index}
          {@const optionErrorMessage = formatZodErrorMessage(validtionErrors, `Options.${index}`)}
          <div class="flex items-center space-x-2">
            <Input type="text" id="option" placeholder="Enter option" bind:value={options[index]} />
            {#if index > 1}
              <Button
                variant="destructive"
                size="sm"
          ...
          {#if optionErrorMessage}
            <p class="text-xs text-red-500">{optionErrorMessage}</p>
          {/if}
        {/each}
      </div>
      ...
      <div class="flex justify-end space-x-2">
        ...
        <Button variant="default" size="sm" class="w-full" onclick={onCreatePoll}
          >Create Poll</Button
        >
      </div>
    </div>
