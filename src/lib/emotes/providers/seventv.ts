...
export class SevenTv extends Provider {
  constructor() {
    super('7TV Global', SevenTvIcon, [], 'sevenTvGlobal');
  }

  override async fetchEmotes(): Promise<Emote[]> {
    try {
      const resp = await deduplicatedFetch('https://7tv.io/v3/emote-sets/global');
      if (resp.status !== 200) {
...
export class SevenTvUser extends Provider {
  ...
  constructor(userId: string) {
    super('7TV User', SevenTvIcon, [], 'sevenTvUser');
    this.userId = userId;
  }
  ...
  override async fetchEmotes(): Promise<Emote[]> {
    ...
    try {
      const resp = await deduplicatedFetch(`https://7tv.io/v3/users/twitch/${this.userId}`);
      if (resp.status !== 200) {
        console.warn('Failed to fetch', this.name);
        return [];
      }
